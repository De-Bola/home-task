...
public interface FuelTabRepository extends JpaRepository<FuelTab, Long> {
    ...
    @Query("select new com.fuel.consumption.dtos.pojos.MonthlyStats(fuelType, sum(purchasedVolume), avg(amountPaid), sum(amountPaid), monthAndYear) from FuelTab where fuelType=:fuelType group by monthAndYear")
    List<MonthlyStats> getMonthlyStatistics(String fuelType);

    @Query("select new com.fuel.consumption.dtos.pojos.MonthlyStats(fuelType, sum(purchasedVolume), avg(amountPaid), sum(amountPaid), monthAndYear) from FuelTab where driverID=:driverId and fuelType=:fuelType group by monthAndYear")
